...
static model_t *Mod_FindName(const c8 *name)
{
	...
	if(i == mod_numknown){
		...
		SDL_strlcpy(mod->name, name, MAX_QPATH);
		mod->needload = 1;
		mod_numknown++;
	}
	return mod;
...
{ // johnfitz -- replaced with lit support code via lordhavoc
	...
	SDL_strlcat(litfilename, ".lit", sizeof(litfilename));
...
static void Mod_LoadEntities(lump_t *l)
{
	...
	SDL_strlcpy(basemapname, loadmodel->name, sizeof(basemapname));
...
static void Mod_LoadFaces(lump_t *l, bool bsp2)
{
	...
	for(s32 surfnum = 0; surfnum < count; surfnum++, out++) {
		...
		if(!SDL_strncasecmp(out->texinfo->texture->name,"sky",3))
...
static FILE *Mod_FindVisibilityExternal()
{
	...
	while((r=fread(&header,1,VISPATCH_HEADER_LEN,f))==VISPATCH_HEADER_LEN){
		...
		if(!SDL_strcasecmp(header.mapname, shortname)) break;
