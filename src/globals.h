...
void Sbar_CalcPos();
s32 Sys_FileType(const c8 *path);                                       // sys.h
...
qpic_t *Draw_PicFromWad(c8 *name);
...
void SCR_CenterPrint(const c8 *str);
void SCR_BeginLoadingPlaque();
void SCR_EndLoadingPlaque();
s32 SCR_ModalMessage(c8 *text, s32 adj);
...
EX c8 my_tcpip_address[NET_NAMELEN];
void NET_Init();
void NET_Shutdown();
struct qsocket_s *NET_CheckNewConnections();
struct qsocket_s *NET_Connect(const c8 *host);
...
s32 PR_AllocString(s32 bufferlength, c8 **ptr);
void PR_Profile_f();
edict_t *ED_Alloc();
...
void Cbuf_Execute();
cmd_function_t *Cmd_AddCommand2 (const c8 *cmd_name, xcommand_t function, cmd_source_t srctype, bool qcinterceptable);
...
sfx_t *S_PrecacheSound(const c8 *sample);
void S_TouchSound(const c8 *sample);
...
c8 *Key_KeynumToString(s32 keynum);
...
void SV_ClientThink();
void SV_ClientPrintf(const c8 *fmt, ...);
...
EX c8 com_token[1024];
EX s32 com_argc;
EX c8 **com_argv;
EX s32 msg_readcount;
EX bool msg_badread;
EX c8 key_lines[32][MAXCMDLINE];
EX s32 edit_line;
EX s32 key_linepos;
...
s32 COM_CheckParm(const c8 *parm);
void COM_Init();
...
size_t LOC_Format(const c8*format,const c8*(*getarg_fn)(s32 idx,void*userdata),void*userdata,c8*out,size_t len);
EX s32 com_filesize;
EX c8 com_basedir[MAX_OSPATH];
EX c8 com_gamedir[MAX_OSPATH];
...
u8 *COM_LoadMallocFile(const c8 *path, u32 *path_id);
...
EX s32 d_pix_min, d_pix_max, d_pix_shift;                          // d_modech.c
...
EX s32 key_count;
EX c8 *keybindings[256];
EX c8 chat_buffer[32];
EX u8 *litwater_base;                                                // d_scan.c
...
EX u8 uipal[768];
EX c8 worldpalname[MAX_OSPATH];
EX c8 worldcmapname[MAX_OSPATH];
EX c8 uipalname[MAX_OSPATH];
EX void SetWorldPal(c8 *path, c8 *cmappath);
...
u8 *Image_LoadImage(const c8 *name, s32 *width, s32 *height);         // image.c
bool nameInList(const c8 *list, const c8 *name);                      // model.c
