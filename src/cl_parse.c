...
void CL_ParseServerInfo()
{
	...
	for(;; nummodels++){ // precache models
		...
		SDL_strlcpy(model_precache[nummodels], str, MAX_QPATH);
		Mod_TouchModel(str);
	}
	...
	for(;; numsounds++){
		...
		SDL_strlcpy(sound_precache[numsounds], str, MAX_QPATH);
		S_TouchSound(str);
	}
...
//backbuffer overflows that would give a false positive.
	...
	case svc_lightstyle:
		...
		SDL_strlcpy(cl_lightstyle[i].map, MSG_ReadString(),
				MAX_STYLESTRING);
	...
	case svc_updatename:
		...
		SDL_strlcpy(cl.scores[i].name, MSG_ReadString(),
				MAX_SCOREBOARDNAME);
		break;
	...
	case svc_cutscene:
		...
		break;
	case svc_sellscreen:
		if(registered.value) Cmd_ExecuteString("help", src_command);
		else cls.sellscreen = 1;
		break;
