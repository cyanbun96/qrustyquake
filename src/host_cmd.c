...
{ // map <servername> command from console. Active clients are kicked off.
	...
	if(cls.state != ca_dedicated){
		...
		for(s32 i = 2; i < Cmd_Argc(); i++){
			SDL_strlcat(cls.spawnparms, Cmd_Argv(i), MAX_MAPSTRING);
			SDL_strlcat(cls.spawnparms, " ", MAX_MAPSTRING);
		}
...
// https://celephais.net/board/view_thread.php?id=60452&start=3666
	...
	memcpy(text+22, kills, (s32)strlen(kills));
...
static void Host_Name_f()
{
	...
	if(Cmd_Argc() == 2) SDL_strlcpy(newName, Cmd_Argv(1), sizeof(newName));
	else SDL_strlcpy(newName, Cmd_Args(), sizeof(newName));
	...
	if(host_client->name[0] && strcmp(host_client->name, "unconnected")
		&& strcmp(host_client->name, newName) != 0)
...
void Host_Say(bool teamonly)
{
	...
	s32 j = sizeof(text)-2-(s32)strlen(text); // -2 for /n and null terminator
	if((s32)strlen(p) > j) p[j] = 0;
	strcat(text, p);
	strcat(text, "\n");
	j = 0;
	for(client_t *client = svs.clients; j < svs.maxclients; j++, client++){
...
void Host_Tell_f()
{
	...
	for(client_t *client = svs.clients; j < svs.maxclients; j++, client++){
		if(!client->active || !client->spawned)
			continue;
		if(SDL_strcasecmp(client->name, Cmd_Argv(1)))
			continue;
		host_client = client;
		SV_ClientPrintf("%s", text);
		break;
...
void Host_Kick_f() // Kicks a user off of the server
{
	...
	if(Cmd_Argc() > 2 && strcmp(Cmd_Argv(1), "#") == 0){
		...
		for(i = 0, host_client = svs.clients; i < svs.maxclients;
			...
			if(SDL_strcasecmp(host_client->name,Cmd_Argv(1))==0)break;
		}
	}
	if(i < svs.maxclients){
		const c8 *who;
		...
		if(Cmd_Argc() > 2){
			...
			if(byNumber){
				...
				while(*message == ' ') // skip white space
					message++;
				message += (s32)strlen(Cmd_Argv(2)); // skip number
			}
...
void FileList_Add(const char *name, const char *desc, filelist_item_t **list)
{
	...
	if(desc) SDL_strlcpy(item->desc, desc, sizeof(item->desc));
	// insert each entry in alphabetical order
	if(*list == NULL || SDL_strcasecmp(item->name, (*list)->name) < 0){
		item->next = *list; //insert at front
		*list = item;
	} else { //insert later
		prev = *list;
		cursor = (*list)->next;
		while(cursor && (SDL_strcasecmp(item->name, cursor->name) > 0)){
			prev = cursor;
			cursor = cursor->next;
		}
		item->next = prev->next;
...
void ExtraMaps_Add(const c8 *name, const c8 *game)
{
	...
	if(!Mod_LoadMapDescription(buf, sizeof(buf), name))
		return;
	if(strncmp(COM_SkipPath(game), "id1", 4))
